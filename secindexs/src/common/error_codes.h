...
template <>
inline bool ErrorCodes::isA<ErrorCategory::RetriableError>(Error code) {
    return isRetriableError(code);
}
...
template <>
constexpr inline bool isNamedCode<ErrorCodes::ShardKeyTooBig> = true;
template <>
...
template <>
struct ErrorCategoriesForImpl<ErrorCodes::BalancerInterrupted> {
    using type = CategoryList<
        ErrorCategory::RetriableError
        >;
};
